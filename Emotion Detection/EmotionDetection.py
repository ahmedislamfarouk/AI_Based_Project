...
def get_face_mesh():
    global _face_mesh_instance, _face_mesh_failed
    ...
    try:
        ...
        _face_mesh_instance = fm
        return fm
    except Exception as e:
        print(f"[EmotionDetection] MediaPipe FaceMesh init failed (likely no EGL): {e}")
        _face_mesh_failed = True
        return None
...
def analyze_faces_and_draw(frame, face_mesh=None):
    ...
    Tiers: 1) MediaPipe+DeepFace 2) Haar+DeepFace 3) Haar heuristic
    """
    if face_mesh is None:
        face_mesh = get_face_mesh()

    # Tier 1: MediaPipe + DeepFace (best accuracy)
    if face_mesh is not None:
        try:
            ...
            print(f"[EmotionDetection] MediaPipe processing failed: {e}")

    # Tier 2: Haar cascade + DeepFace (no EGL needed)
    ...
    if len(faces) > 0:
        ...
        if _deepface_available:
            ...
            raw_probs = {}
            try:
                result = DeepFace.analyze(face_crop, actions=['emotion'], enforce_detection=False, detector_backend='skip', silent=True)
                emotion = result[0]['dominant_emotion']
                ...
                try:
                    result = DeepFace.analyze(face_crop, actions=['emotion'], enforce_detection=False, detector_backend='opencv', silent=True)
                    emotion = result[0]['dominant_emotion']
                    raw_probs = result[0].get('emotion', {})
                    max_prob = max(raw_probs.values()) if raw_probs else 0
                    if max_prob < 0.25:
            ...
            emotion = "Anxious" if face_ratio > 0.4 else "Calm"

        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv2.putText(frame, f"Emotion: {emotion}", (x, y - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        states.append(emotion)
    return frame, states
