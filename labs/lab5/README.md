...
| **Function (Parsing)** | `parser.py` | Extracts data from your text. |
| **Email (IMAP)** | `email_service.py` | Connects to inbox to read emails. |
| **Groq/AI Node** | `llm_service.py` | Sends data to AI for analysis. |
| **Email (SMTP)** | `email_service.py` | Sends final report to administrator. |
| **Workflow Root** | `main.py` | The "Start" button for the project. |
...
* **`llm_service.py`**: Connects to Groq for analysis.
* **`parser.py`**: Cleans and prepares the data.
* **`.env`**: Stores your API keys and passwords safely.
